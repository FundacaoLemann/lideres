...
class LVCA_FAQ {
    ...
    public function shortcode_func($atts, $content = null, $tag) {
        ...
        $output = '<div class="lvca-faq-list lvca-' . $style . ' lvca-grid-container ' . lvca_get_grid_classes($settings) . '">';

        $output .= do_shortcode($content);

        $output .= '</div>';

        return apply_filters('lvca_faq_output', $output, $content, $settings);
    }
    ...
    public function child_shortcode_func($atts, $content = null, $tag) {
        ...
        $output = '<div class="lvca-grid-item lvca-faq-item ' . $animate_class . '" ' . $animation_attr . '>';

        $output .= '<h3 class="lvca-faq-question">' . wp_kses_post($question) . '</h3>';

        $output .= '<div class="lvca-faq-answer">' . wp_kses_post($content) . '</div>';

        $output .= '</div>';

        return apply_filters('lvca_faq_item_output', $output, $content, $settings);
