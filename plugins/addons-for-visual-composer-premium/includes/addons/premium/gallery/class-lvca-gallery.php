...
class LVCA_Gallery {
    ...
    public function shortcode_func($atts, $content = null, $tag) {
        ...
        if (!empty($items)) :
            ...
            $output = '<div class="lvca-gallery-wrap lvca-gapless-grid" data-settings=\'' . wp_json_encode($settings)
            ...
            if (!empty($settings['heading']) || $settings['filterable']):
                ...
                $gallery_header = '<div class="lvca-gallery-header ' . $header_class . '">';

                if (!empty($settings['heading'])):

                    $gallery_header .= '<h3 class="lvca-heading">' . wp_kses_post($settings['heading']) . '</h3>';
                ...
                $output .= apply_filters('lvca_gallery_header', $gallery_header, $settings);

            endif;

            $output .= '<div id="' . uniqid('lvca-gallery') . '"
                     class="lvca-gallery js-isotope lvca-' . esc_attr($settings['layout_mode']) . ' lvca-grid-container ' . lvca_get_grid_classes($settings) . $settings['gallery_class'] . '"
                     data-isotope-options=\'{ "itemSelector": ".lvca-gallery-item", "layoutMode": "' . esc_attr($settings['layout_mode']) . '", "masonry": { "columnWidth": ".lvca-grid-sizer" } }\'>';

            if ($settings['layout_mode'] == 'masonry'):

                $output .= '<div class="lvca-grid-sizer"></div>';
            ...
            $output .= '</div><!-- .lvca-gallery -->';

            $output .= $common->paginate_gallery($items, $settings);

            $output .= '</div><!-- .lvca-gallery-wrap -->';

        endif;

        return apply_filters('lvca_gallery_output', $output, $content, $settings);
    }
