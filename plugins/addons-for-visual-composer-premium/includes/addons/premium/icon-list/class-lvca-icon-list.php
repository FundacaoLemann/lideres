...
class LVCA_Icon_List {
    ...
    public function shortcode_func($atts, $content = null, $tag) {
        ...
        $output = '<div id="lvca-icon-list' . uniqid()
            . '" data-settings=\'' . htmlspecialchars(json_encode($this->settings))
            . '\' class="lvca-icon-list lvca-align' . $this->settings['align'] . '">';

        $output .= do_shortcode($content);

        $output .= '</div>';

        return apply_filters('lvca_icon_list_output', $output, $content, $this->settings);
    }
    ...
    public function child_shortcode_func($atts, $content = null, $tag) {
        ...
        $output = '<div class="lvca-icon-list-item' . $animate_class . '" ' . $animation_attr . ' title = "' . $title . '">';
        ...
        if ($icon_type == 'icon_image') :
            ...
            if (empty($href)) :
                ...
                $output .= wp_get_attachment_image($icon_image, 'full', false, array('class' => 'lvca-image full', 'alt' => $title));

                $output .= '</div>';

            else :

                $output .= '<a class="lvca-image-wrapper" href="' . esc_url($href) . '" ' . $target . '>';
        ...
        elseif (!empty(${'icon_' . $icon_family})) :
            ...
            if (empty($href)) :
                ...
                $output .= '<a class="lvca-icon-wrapper" href="' . esc_url($href) . '" ' . $target . '>';
        ...
        return apply_filters('lvca_icon_list_item_output', $output, $content, $settings);
