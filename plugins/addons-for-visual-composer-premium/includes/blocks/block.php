...
abstract class LVCA_Block {
    ...
    function init($settings) {
        ...
        if (function_exists('vc_build_loop_query')) {
            ...
            $posts_query = apply_filters('lvca_block_' . $this->settings['block_id'] . '_posts_query', $posts_query, $this->settings);
            ...
            $query_args = apply_filters('lvca_block_' . $this->settings['block_id'] . '_query_args', query_args, $this->settings);
    ...
    function render($settings) {
        ...
        $grid_classes = apply_filters('lvca_' . $settings['block_type'] . '_grid_classes', $grid_classes, $settings);

        // add container class to enable column styling
        $output .= '<div class="lvca-block-inner lvca-grid-container '. $grid_classes . '">';

        $block_output = $this->inner($this->wp_query->posts, $this->settings);

        $output .= apply_filters('lvca_' . $this->settings['block_type'] . '_inner_output', $block_output, $this);
        ...
        return apply_filters('lvca_' . $this->settings['block_type'] . '_output', $output, $this);

    }

    // get atts
    private function get_block_data_atts() {
        ...
        return apply_filters('lvca_block_data_attributes', $output, $this);

    }

    private function get_settings_data_atts() {
        ...
        return apply_filters('lvca_block_settings_data_attributes', $data_atts, $this->settings);
    ...
    private function get_block_header() {
        ...
        return apply_filters('lvca_' . $this->settings['header_template'] . '_output', $output, $this->block_header_obj);
    ...
    private function get_block_filter_terms() {
        ...
        return apply_filters('lvca_block_filter_terms', $block_filter_terms, $this);
    ...
    function get_block_pagination() {
        ...
        return apply_filters('lvca_block_pagination', $output, $this);
    }
    ...
    function get_column_class($column_size = 3) {
        ...
        return apply_filters('lvca_block_column_class', $style_class, $column_size, $this->settings);
    }

    function get_grid_classes_from_settings_field($settings, $columns_field) {
    ...
    protected function get_block_classes($classes_array = array()) {
        ...
        $block_classes[] = apply_filters('lvca_' . $this->settings['block_type'] . '_class', $this->get_block_class(), $this);

        // add block header type for styling
        $block_classes[] = apply_filters('lvca_' . $this->settings['header_template'] . '_class', $this->block_header_obj->get_block_header_class(), $this->block_header_obj);
        ...
        $block_classes = apply_filters('lvca_block_classes', $block_classes, $this);
