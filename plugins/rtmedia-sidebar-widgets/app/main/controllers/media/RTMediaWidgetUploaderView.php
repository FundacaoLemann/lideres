...
/**
 * Description of RTMediaWidgetUploaderView
 *
 * @author sanket
 */
class RTMediaWidgetUploaderView {
	...
	/**
	 * Genrates rtmedia_upload_nonce
	 * @param boolean $echo true for print nonce fields and false for nonce value
	 * @param type $only_nonce
	 * @return type
	 */
	...
	public function render ( $arguments) {
		...
		$tabs = array(
			'file_upload' => array(
				'default' => array( 'title' => __( 'File Upload', 'rtmedia' ), 'content' => '<div id="rtmedia-upload-container-'.$widgetid.'" ><div id="drag-drop-area-'.$widgetid.'" class="drag-drop widget-drag-drop">' . $album_el . '<input id="rtMedia-upload-button-'.$widgetid.'" value="' . __( 'Select', 'rtmedia' ) . '" type="button" class="rtmedia-upload-input rtmedia-file" />'.$context_el.  '</div><ul class="plupload_filelist_content ui-sortable rtm-plupload-list clearfix rtmp-uploaded-file rtMedia-queue-list" id="rtmedia_uploader_filelist-'.$widgetid.'"></ul></div>' ),
			),
	...
	/**
	 *  Loads scripts and localize parameters
	 * @global type $rtmedia
	...
	public function register_scripts( $widget_id, $arguments ) {
		...
		wp_enqueue_script( 'rtmedia-widget-backbone', RTMEDIA_WIDGETS_URL . 'app/assets/js/widget_uploader.js', array( 'plupload', 'backbone' ), false, false );
		...
		$params = array(
			...
			'multipart_params' => apply_filters( 'rtmedia-multi-params', array( 'redirect' => 'no', 'action' => 'wp_handle_upload', '_wp_http_referer' => $_SERVER['REQUEST_URI'], 'mode' => 'file_upload', 'rtmedia_upload_nonce' => RTMediaWidgetUploaderView::upload_nonce_generator( false, true ) ) ),
	...
	protected function locate_template( $template ) {
		...
		if ( file_exists( RTMEDIA_WIDGETS_PATH . 'templates/' . $template_name ) ) {
			$located = RTMEDIA_WIDGETS_PATH . 'templates/' . $template_name;
		}
